	...
	if (hit_type == 0 && stage.prefs.notesplashes)
	{
		for (int i = 0; i < 3; i++)
		{
			...
			Obj_Splash *splash = Obj_Splash_New(
				this->position.x + this->notes[type].x,
				(this->position.y + this->notes[type].y) * (stage.prefs.downscroll ? -1 : 1),
				type & 0x3
			);
			if (splash != NULL)
...
{
	//Perform note check
	for (Note *note = this->chart_cur_note;; note++)
	{
		...
		if (playing)
		{
			...
			for (u8 j = 0; j < 4; j++)
			{
				if (hit[j] & 5)
				{
					...
					Stage_SustainCheck(this, j);
				}
				if (hit[j] & 1)
				{
					this->pad_press |= stage.prefs.control_keys[j];
					Stage_NoteCheck(this, j);
				}
			}
...
static void Stage_DrawStrum(PlayerState *player, u8 i, RECT *note_src, RECT_FIXED *note_dst)
{
	(void)note_dst;
...
static void Stage_DrawNotes(PlayerState *player)
{
	...
	while (scroll.start_step > player->chart_cur_note->pos)
	{
	...
	//Draw notes for this specific player
	for (Note *note = player->chart_cur_note; note->pos != 0xFFFF; note++)
	{
		//Update scroll
		...
		// Use individual note Y position from player->notes array
		u8 note_index = note->type & 0x3; // 0-3 for the 4 lanes
		fixed_t y = player->notes[note_index].y + FIXED_MUL(stage.speed, time * 150);
		...
		if (y < FIXED_DEC(-40, 1))
		{
			...
			if (!(note->type & (player->bot | NOTE_FLAG_HIT)))
			{
				//Missed note
				Stage_CutVocal();
				Stage_MissNote(player, note->type);
				NoteMissEvent(note->type, 0);
				player->health -= 475;
			}
			
			//Update current note
			player->chart_cur_note++;
		}
		else
		{
			...
			RECT note_src;
			RECT_FIXED note_dst;
			if (y > (FIXED_DEC(400,2) + scroll.size) || note->pos == 0xFFFF)
				break;
			
			// Use individual note X position from player->notes array
			fixed_t note_x = player->notes[note_index].x;
			...
			if (note->type & NOTE_FLAG_SUSTAIN)
			{
				...
				fixed_t clip;
				y -= scroll.size;
				if ((note->type & (player->bot | NOTE_FLAG_HIT)) || ((player->pad_held & stage.prefs.control_keys[note->type & 0x3]) && (note_fp + stage.late_sus_safe >= stage.note_scroll)))
				{
					clip = player->notes[note_index].y - y;
					if (clip < 0)
						clip = 0;
				}
				else
				...
				if (note->type & NOTE_FLAG_SUSTAIN_END)
				{
					if (clip < (24 << FIXED_SHIFT))
					{
						...
						note_dst.x = player->position.x + note_x - FIXED_DEC(16,1);
						note_dst.y = player->position.y + y + clip;
				...
				else
				{
					...
					fixed_t next_y = player->notes[note_index].y + FIXED_MUL(stage.speed, next_time * 150) - scroll.size;
			...
			else
			{
				...
				note_dst.x = player->position.x + note_x - FIXED_DEC(16,1);
				note_dst.y = player->position.y + y - FIXED_DEC(16,1);
				note_dst.w = note_src.w << FIXED_SHIFT;
				note_dst.h = note_src.h << FIXED_SHIFT;
	...
	//Draw note HUD for this specific player
	RECT note_src = {0, 0, 32, 32};
	RECT_FIXED note_dst = {0, 0, FIXED_DEC(32,1), FIXED_DEC(32,1)};
		
	for (u8 i = 0; i < 4; i++)
	{
		// Use individual note positions for HUD
		note_dst.x = player->position.x + player->notes[i].x - FIXED_DEC(16,1);
		note_dst.y = player->position.y + player->notes[i].y - FIXED_DEC(16,1);
		
		if (stage.prefs.downscroll)
			note_dst.y = -note_dst.y - note_dst.h;
				
		Stage_DrawStrum(player, i, &note_src, &note_dst);
		Stage_DrawTex(&stage.tex_hud0, &note_src, &note_dst, &stage.camera.hud);
	}
}
...
static void Stage_LoadChart(void)
{
	...
	u8* section_address = chart_byte + 8;
	u16 section_size = *((u16*)(chart_byte + 4));
	u16 note_0_size = *((u16*)(chart_byte + 6));
	
	//Directly use section and notes pointers
	stage.sections = (Section*)(section_address);
	stage.player_state[0].chart_notes = (Note*)(chart_byte + section_size);
	stage.player_state[1].chart_notes = (Note*)(chart_byte + section_size + note_0_size);
	
	stage.player_state[0].chart_cur_note = stage.player_state[0].chart_notes;
	stage.player_state[1].chart_cur_note = stage.player_state[1].chart_notes;
	
	for (Note *note = stage.player_state[0].chart_notes; note->pos != 0xFFFF; note++)
		stage.player_state[0].chart_num_notes++;
...
static void Stage_LoadState(void)
{
	...
	stage.player_state[0].bot = false;
...
static void Stage_InitPlayerState(PlayerState *this)
{
	this->position.x = FIXED_DEC(80,1);
	this->position.y = FIXED_DEC(-80,1);
	
	for (int i = 0; i < 4; i++)
	{
		this->notes[i].x = FIXED_DEC(33 * (i - 2),1) + FIXED_DEC(16,1);
		this->notes[i].y = FIXED_DEC(0,1);
	}
}
...
void Stage_Load(StageId id, StageDiff difficulty, boolean story)
{
	...
	Stage_InitPlayerState(&stage.player_state[0]);
	Stage_InitPlayerState(&stage.player_state[1]);
	
	u8 swap = (stage.prefs.mode == StageMode_Swap);
	stage.player_state[0].position.x = FIXED_DEC(80 - 160 * swap, 1);
	stage.player_state[1].position.x = FIXED_DEC(-80 + 160 * swap, 1);
}
...
void Stage_Tick(void)
{
	...
	switch (stage.state)
	{
		case StageState_Play:
		{
			...
			//Draw stage notes
			Stage_DrawNotes(&stage.player_state[0]);
			Stage_DrawNotes(&stage.player_state[1]);
