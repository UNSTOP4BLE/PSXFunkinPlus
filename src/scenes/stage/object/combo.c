...
boolean Obj_Combo_Tick(Object *obj)
{
	...
	if (this->hit_type != 0xFF && this->ht < (FIXED_DEC(16,1) / 60))
	{
		...
		Stage_DrawTex(&stage.tex_hud0, &hit_src, &hit_dst, &stage.camera.hud);
	...
	if (this->numt < (FIXED_DEC(16,1) / 60))
	{
		for (u8 i = 0; i < 5; i++)
		{
			...
			Stage_DrawTex(&stage.tex_hud0, &num_src, &num_dst, &stage.camera.hud);
...
Obj_Combo *Obj_Combo_New(fixed_t x, fixed_t y, u8 hit_type, u16 combo)
{
	...
	if ((x >= 0) ^ (stage.prefs.mode < StageMode_2P))
		this->x = FIXED_DEC(stage.prefs.combox,1) - FIXED_DEC(SCREEN_WIDEADD,4);
	else
		this->x = FIXED_DEC(-stage.prefs.combox - 60,1) + FIXED_DEC(SCREEN_WIDEADD,4);
	y = FIXED_DEC(stage.prefs.comboy,1);
